...
class NotaServiceTest {
    ...
    void deveAceitarNotaIgualAZero() {
        ...
        notaDTO.setAlunoId(1L);
        notaDTO.setAvaliacaoId(1L);
        notaDTO.setValor(0.0);

        when(alunoRepository.findById(1L)).thenReturn(Optional.of(aluno));
        when(avaliacaoRepository.findById(1L)).thenReturn(Optional.of(avaliacao1));
        when(notaRepository.findByAlunoIdAndAvaliacaoId(1L, 1L)).thenReturn(Optional.empty());
    ...
    void deveLancarExcecaoAlunoNaoEncontrado() {
        NotaDTO notaDTO = new NotaDTO();
        notaDTO.setAlunoId(999L);
        notaDTO.setAvaliacaoId(1L);
        notaDTO.setValor(8.0);
        ...
        ResponseStatusException exception = assertThrows(
            ResponseStatusException.class,
            () -> notaService.salvar(notaDTO),
            "Deve lançar exceção para aluno inexistente"
        );
    ...
    void deveLancarExcecaoAvaliacaoNaoEncontrada() {
        ...
        ResponseStatusException exception = assertThrows(
            ResponseStatusException.class,
            () -> notaService.salvar(notaDTO),
            "Deve lançar exceção para avaliação inexistente"
        );

        assertTrue(exception.getReason().contains("Avaliação não encontrada"));
    }
    ...
    void deveAtualizarNotaExistente() {
        ...
        notaDTO.setAlunoId(1L);
        notaDTO.setAvaliacaoId(1L);
        notaDTO.setValor(9.5);

        nota1.setValor(8.0); // Valor antigo
        
        when(alunoRepository.findById(1L)).thenReturn(Optional.of(aluno));
        when(avaliacaoRepository.findById(1L)).thenReturn(Optional.of(avaliacao1));
        when(notaRepository.findByAlunoIdAndAvaliacaoId(1L, 1L)).thenReturn(Optional.of(nota1));
        when(notaRepository.save(any(Nota.class))).thenReturn(nota1);
    ...
    void deveRejtarLoteSeFirstNotaInvalida() {
        ...
        ResponseStatusException exception = assertThrows(
            ResponseStatusException.class,
            () -> notaService.salvarEmLote(notasDTOs),
            "Deve rejeitar lote com primeira nota inválida"
        );
    ...
    @Test
    @DisplayName("Deve retornar lista vazia quando aluno não tem notas")
    void deveRetornarListaVaziaQuandoSemNotas() {
        when(notaRepository.findByAlunoId(1L)).thenReturn(new ArrayList<>());
